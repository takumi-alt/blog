            ...
            <main className="bg-gray-200 text-center">
                <div className="lg:w-4/5 bg-gray-200 mx-auto">
                    <ul className="w-full mb-0 list-none flex flex-wrap lg:justify-between justify-center">
                        {
                            posts.map(post => <li key={post.id} className="bg-regal-white w-58 h-48 sm:w-84 sm:h-64 m-4 sm:m-8 shadow-purple3 transition duration-500 rounded-xl transform hover:translate-x-4 hover:translate-y-4 hover:shadow-zero">
                                <Link to={`/blog/article/${post.path}`} className="h-34 sm:h-45 w-full items-end justify-center rounded-xl">
                                    ...
                                    <span className="text-regal-black bg-rgba w-full font-semibold text-sm sm:text-base h-14 sm:h-18 absolute bottom-0 left-0 rounded-b-xl">{post.title}</span>
                                </Link>
                                </li>)
                        }
                    </ul>
